...
import {
  ...
  computeTotalSpeed,
  type FrameInfoWithPercentage,
  type FrameId,
} from '../hooks/useFrameInfo';
import type { ShareCardData } from '../share/createShareCard';
...
interface FrameInfoModalProps {
  frameId: string | null;
  frameData: FrameInfoWithPercentage | null;
  onClose: () => void;
...
YOUR CURRENT VALUES
-------------------
Distance traveled: ${formatDistanceCompactUnified(frameData.distanceKm, distanceUnit)}
Current speed: ${formatSpeedWithUnit(frameData.speedKms, speedUnit)}
...
---
Generated by Spacetime Journey
${appUrl}
`.trim();
    ...
    try {
      ...
      try {
        ...
        if (success) {
          ...
          // execCommand copy failed silently
        }
      } catch (_fallbackError) {
        // Both clipboard methods failed, ignore silently
      }
    }
  };
            ...
            <div className="grid grid-cols-3 gap-4">
              <div>
                <div className="text-xl font-bold text-white tabular-nums">
                  {formatDistanceCompactUnified(frameData.distanceKm, distanceUnit)}
                </div>
              ...
              <div>
                <div className="text-xl font-bold text-white tabular-nums">
                  {formatSpeedWithUnit(frameData.speedKms, speedUnit)}
                </div>
                <div className="text-[10px] text-neutral-500">
...
interface JourneyModalProps {
  ...
  locale?: string;
  /** Canvas element for screenshot capture (share feature) */
  canvasRef?: HTMLCanvasElement | null;
  /** App URL for branding in share card and exports */
  appUrl?: string;
  /** App name for branding in share card */
  appName?: string;
  /** Callback to prepare scene for share (e.g., focus on Earth with birthplace). Returns true if changes were made. */
  onPrepareShare?: () => Promise<boolean>;
  /** User's birth date for display on share card */
  birthDate?: Date | null;
}
...
/**
 * Planet display metadata (colors and icons for UI).
 * Orbital periods are imported from core constants.
 */
const PLANET_DISPLAY: Record<string, { color: string; icon: string }> = {
  Mercury: { color: '#b5b5b5', icon: '/textures/2k_mercury.jpg' },
  Venus: { color: '#e6c87a', icon: '/textures/2k_venus_surface.jpg' },
  Earth: { color: '#4488cc', icon: '/textures/earth_daymap.jpg' },
  Mars: { color: '#c1440e', icon: '/textures/2k_mars.jpg' },
  Jupiter: { color: '#d4a574', icon: '/textures/2k_jupiter.jpg' },
  Saturn: { color: '#f4d59e', icon: '/textures/2k_saturn.jpg' },
  Uranus: { color: '#b5e3e3', icon: '/textures/2k_uranus.jpg' },
  Neptune: { color: '#5b7fde', icon: '/textures/2k_neptune.jpg' },
};
...
function buildFrameInfo(
  ...
  const total = computeTotalDistance(worldline);

  const frameIds: FrameId[] = ['spin', 'orbit', 'galaxy', 'cmb'];
  const frameData: Record<FrameId, { label: string; description: string }> = {
    spin: {
  ...
  const frames = frameIds.map((id) => ({
    id,
    label: frameData[id].label,
    description: frameData[id].description,
    distanceKm: worldline.distances[id].distanceKm,
    speedKms: worldline.frames[id].velocityKms,
...
export function JourneyModal({
  ...
  const t = getUIContent(locale ?? 'en');
  const [distanceUnit, setDistanceUnit] = useState<DistanceUnit>('km');
  const [infoFrameId, setInfoFrameId] = useState<string | null>(null);
  const [isPreparing, setIsPreparing] = useState(false);
  const { shareJourney, isSharing, canShare } = useShareJourney({
    locale,
    appUrl,
    appName,
  });
  ...
  const handleShare = useCallback(async () => {
    if (!canvasRef || !worldline || !age) return;

    // Prepare scene (focus on Earth with birthplace)
    if (onPrepareShare) {
      setIsPreparing(true);
      const needsUpdate = await onPrepareShare();

      if (needsUpdate) {
        // Wait for React state update + R3F render cycle to complete
        // Double requestAnimationFrame ensures the frame has actually been painted
    ...
    const frameInfo = buildFrameInfo(worldline, t);
    ...
    await shareJourney(canvasRef, shareData);
  }, [canvasRef, worldline, age, birthDate, t, locale, shareJourney, onPrepareShare]);

  if (!isOpen) return null;

  const frames = buildFrameInfo(worldline, t);
  ...
  // Fun statistics
  const moonTrips = totalDistanceKm / (MOON_MEAN_DISTANCE_KM * 2);
  const plutoTrips = totalDistanceKm / PLUTO_MEAN_DISTANCE_KM;
  ...
  const solarOrbits = ageInDays / JULIAN_YEAR_DAYS;

  // Planetary ages
  const planetaryAges = Object.entries(ORBITAL_PERIODS_DAYS).map(([planet, days]) => {
    const display = PLANET_DISPLAY[planet] ?? { color: '#ffffff', icon: '' };
    const ageOnPlanet = ageInDays / days;
    return {
      planet,
      age: ageOnPlanet,
      color: display.color,
  ...
  const distanceFormatted = formatDistanceFullWords(
    totalDistanceKm,
    distanceUnit,
    t.units
  );
                ...
                <div className="text-xs sm:text-sm text-neutral-400 mt-3 flex flex-col sm:flex-row items-center justify-center gap-1 sm:gap-4">
                  <span>
                    ...
                    <span className="text-emerald-400 font-medium">
                      {formatSpeedWithUnit(totalSpeedKms, speedUnit)}
                    </span>
                  </span>
                ...
                <div>
                  <div className="mb-3">
                    ...
                    <div className="text-[10px] text-neutral-500 mt-1">
                      {t.journey.modal.milestonesExplanation}
                    </div>
                  </div>
                  <div className="grid grid-cols-2 gap-3">
                    <div className="p-4 rounded-lg bg-white/5 border border-white/5">
                      <div className="text-2xl font-bold text-blue-400 tabular-nums">
                        {formatStatistic(earthCircumferences)}
                      </div>
                  ...
                  {planetaryAges.map(({ planet, age, color, icon }) => (
                    ...
                    >
                      ...
                      <div className="text-[10px] text-neutral-500 mt-0.5">
                        {t.journey.modal.planetYearsTemplate?.replace(
                          '{planet}',
                          (t.journey.modal.planets as Record<string, string>)?.[planet] ||
                            planet
                        ) || `${planet} years`}
                      </div>
                    </div>
        ...
        <div className="px-4 sm:px-6 py-4 border-t border-white/10 bg-white/5">
          <div className="flex gap-3 mb-3">
            {canvasRef && (
              <ShareButton
                onClick={handleShare}
                isLoading={isPreparing || isSharing}
                canShare={canShare}
                label={t.share?.button || 'Share Journey'}
                downloadLabel={t.share?.downloadButton || 'Download Image'}
                generatingLabel={t.share?.generating || 'Creating image...'}
              />
            )}
            <button
              onClick={onClose}
              className="flex-1 py-3 rounded-xl bg-gradient-to-r from-blue-600 to-purple-600 hover:from-blue-500 hover:to-purple-500 text-white font-medium transition-all shadow-lg shadow-blue-500/25 flex items-center justify-center gap-2"
            >
              <svg
                className="w-5 h-5"
                fill="none"
